(function () {
  ...
  const ImageUtils = {
    getImagePath(filename) {
      ...
      if (filenameStr.includes("localhost:8080/media/")) {
        const match = filenameStr.match(/\/media\/root\/demo-webshop\/.+$/);
        if (match) return match[0];
      }
      if (filenameStr.startsWith("/media/")) return filenameStr;
      if (filenameStr.startsWith("http")) return filenameStr;
      return `/media/root/demo-webshop/${filenameStr}`;
    }
  };
  ...
  const Utils = {
    ...
    formatMoney(val, currency = "USD") {
      try {
        ...
        if (isNaN(n)) return "";
        return new Intl.NumberFormat(this.lang(), { style: "currency", currency }).format(n);
      } catch {
        return String(val || "");
      }
    },
  ...
  const Product = {
    normalize(p = {}) {
      ...
      const fullImagePath = ImageUtils.getImagePath(rawImage);
  ...
  const Model = {
    ...
    normalizeCart() {
      ...
      cart.items.forEach(item => {
        ...
        const fullPath = ImageUtils.getImagePath(rawImage);
        item.image = fullPath;
  ...
  const Cart = {
    ...
    add(product, qty = 1) {
      ...
      const imageUrl = ImageUtils.getImagePath(rawImage);
      ...
      if (existing) {
        ...
        if (!existing.image || !existing.image.startsWith("/media/")) {
  ...
  const UI = {
    ...
    createCard(product) {
      ...
      if (image) {
        image.setSrc(normalized.image || normalized.imageUrl || "");
      }
  ...
  const Loader = {
    async load() {
      ...
      try {
        ...
        const normalizedItems = Product.normalizeList(items);
        
        Model.setData("ProductsModel", { items: normalizedItems });
  ...
  const ThankYou = {
    dialog: null,
    image: null,
    imageSrc: ImageUtils.getImagePath("Neptune%20DXP%2024.14.0.png"),
      ...
      this.dialog.addEventDelegate({
        onAfterRendering: () => {
          try {
            const $dialog = this.dialog.$();
            $dialog.find(".sapMDialogScroll").css({ padding: "0", margin: "0" });
            $dialog.find(".sapMDialogScrollCont").css({ padding: "0", margin: "0" });
            $dialog.find(".wsThanksImg img").css({ width: "100%", height: "100%", objectFit: "cover", display: "block" });
            $dialog.off("click.wsThanks").on("click.wsThanks", () => this.dialog.close());
          } catch {}
        }
      });
    },
  ...
  window.formatImagePath = filename => ImageUtils.getImagePath(filename);
  ...
  window.onCheckout = () => {
    console.log("Checkout completed", {
      ...
      cartItems: Model.getData("CartModel", {}).items?.length || 0
